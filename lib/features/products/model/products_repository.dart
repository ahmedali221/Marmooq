...
class ProductsRepository {
  ...
  getProducts({bool forceRefresh = false}) async {
    try {
      ...
      for (int i = 0; i < collections.length; i += 3) {
        ...
        final batchFutures = batch.map((collection) => 
          _fetchCollectionProducts(collection, shopifyStore)
        );
        
        final batchResults = await Future.wait(batchFutures);
        collectionResults.addAll(batchResults.where((result) => 
          (result['products'] as List).isNotEmpty
        ));
  ...
  // Optimized product fetching for a single collection
